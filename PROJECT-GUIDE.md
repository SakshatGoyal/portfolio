...
9. `worker/index.ts` and the two Wrangler files — native Static Assets delivery, preview isolation, and Worker identities.
...
├── worker/              the small Cloudflare fallback request layer
...
- `src/data/image-variants.js`, `src/data/video-variants.js`, and `src/styles/generated-image-variants.css` are generated files; change their generators or source manifests, not these outputs by hand.
- Cloudflare Static Assets serves matching files directly, including generated videos, and owns cache validators and byte-range semantics. Delivery probes validate either the platform's complete `200` response or a standards-compliant `206` slice byte-for-byte and reject `416` for valid conditional ranges. `worker/index.ts` is only an `ASSETS.fetch(request)` fallback for unmatched requests.
...
npm run build:media          # regenerate responsive MP4/WebM variants
...
| File | Description | Last Modified | Tokens |
|------|-------------|---------------|--------|
| `worker/index.ts` | Cloudflare Worker fallback that passes unmatched requests to the Static Assets binding. | 2026-08-30 | 30 |
